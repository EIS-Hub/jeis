...
def plot_Vh1(w, batch_spikes, batch_labels, hyperparams, sample_id):
    # plot one figure with 5 plots of 5 neurons in the hidden layer 1, each showing V_h1 over time
    *state, out, loss, pred = (
        out_debug(batch_spikes, batch_labels, hyperparams, w)
    )
    ...
    # shape of V_h1 is (batch_size, n_steps, n_neurons), so in this case (batch_size, 143, 5)
    ...
    fig, axs = plt.subplots(6, 1, figsize=(14, 6), sharex=True, sharey='row')
    for i in range(4): # print input spikes
        axs[0].plot(batch_spikes_to_plot[sample_id, :, i] * i, 'o', markersize=2, color='b')
    for i in range(5): # print membrane potentials of all classes of the first 5 neurons in hidden layer 1
        for class_id in range(4):
            axs[i+1].plot(V_h1[sample_id, :, class_id], label=f'V_h1 class {class_id}')
    axs[0].set_title('V_mem values of all classes of first 5 neurons in hidden layer 1')
    axs[0].set_ylabel('Input spikes')
    axs[1].set_ylabel('V_h1 (N0)')
    axs[2].set_ylabel('V_h1 (N1)')
    axs[3].set_ylabel('V_h1 (N2)')
    axs[4].set_ylabel('V_h1 (N3)')
    axs[5].set_ylabel('V_h1 (N4)')
